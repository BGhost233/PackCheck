...
8. packing 地基层 = L0 + 数据种子（品类→格子分组映射）为第二步统一清单视图预埋
...
### Task 4: 新增格子分组数据种子 GearLoadout.ets（第二步预埋）

**Files:** Create `entry/src/main/ets/constants/GearLoadout.ets`

> 地基层只建表不用，为第二步「带格子的核查清单」零成本预埋（spec §3.5 / §6，纲领 §4 v3）。
...
// 格子分组数据种子（第二步实现统一清单视图时使用，地基层只建表）。纲领 §4 v3 / spec §3.5、§6
...
// 品类 → 默认格子分组（弱映射，用户可手动微调）。key 与 DEFAULT_CATEGORIES 严格一致
...
// 给定品类返回格子分组提示，未命中回退到 Misc/Accessory
...
- [ ] **Step 3: Commit**
```bash
git add -A && git commit -m "feat: 新增格子分组数据种子 GearLoadout（品类→格子映射，为第二步统一清单视图预埋）"
```
...
- **第二步 引擎**：带格子的核查清单统一视图（纲领 §4 v3，第二个灵魂）—— 用本计划 Task 4 种下的 `GearLoadout` 格子分组映射，合并配装/清单为统一界面，格子始终可见 + 空态虚线框 + 打勾核查。
