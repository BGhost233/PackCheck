...
| LoadoutService 纯函数 | LoadoutService.ets | groupByZone 等；~~sortItemsByLayer~~ 一期不再做分层排序（保留签名，改恒等返回 / 按添加顺序） |
...
> 实现要点：`geometryTransition` + `animateTo({ curve: SPRING_CEREMONY }, () => { this.focusedZone = zone })`，进退场对称。详见 §4.5。
...
+-- UnifiedChecklistView (新组件，替代 LoadoutView + ChecklistDetail)
    ...
    |   +-- FAB
    +-- if (focusedZone != null) 全屏聚焦态:
        +-- FocusedZoneView (× + Zone 名 + 进度 + 全量列表 + 底部添加行)
            +-- 共享元素 geometryTransition(zoneId)

每个 item 行（网格态 & 聚焦态共用）:
  Stack {
    Row { CheckMark + 名称 ... }
    Image('').hitTestBehavior(Block)               // 透明手势层
      .gesture(GestureGroup(Sequence, LongPress(400) + Pan(distance 5)))
  }                                                 // 自实现连贯手势，详见 §4.5

根 Stack 顶层（条件渲染）:
  if (overlayItem != null) DragOverlay {           // 自绘浮层（蒙层+详情缩略图+菜单）
...
// item 行上叠透明手势层，避免子元素（CheckMark / 文本）抢事件
Stack() {
  Row() { CheckMark({ ... }); Text(item.name) }
  Image('')
    .width('100%').height('100%')
    .hitTestBehavior(HitTestMode.Block)   // 关键：拦下触摸，子元素不再拦截
    .gesture(
      GestureGroup(
...
// 网格态格子标题 onClick
.onClick(() => {
  this.getUIContext().animateTo({ curve: SPRING_CEREMONY() }, () => {
...
| 全屏聚焦进入/退出 | SPRING_CEREMONY | geometryTransition | 格子放大铺满 / 缩回原位，进退对称 |
