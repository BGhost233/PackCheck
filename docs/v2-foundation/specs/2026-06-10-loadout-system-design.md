...
> - v2 → v3：吸收第二轮审查（Everest），13 项修正。主要变更：① 补充 NavDestinationMap 集成路径；② GearPickerSheet 反馈从「背景动画」改为「Sheet 内实时反馈 + 关闭后汇总动画」；③ 空态改为渐进式引导（Zone 按需出现）；④ FAB 仅配装视图可见；⑤ 临时添加完整流程定义；⑥ 共享信息区布局定义；⑦ Zone 颜色系统；⑧ 勾选即时反馈优化；⑨ 触控区规范。
...
- 其余 6 个 Zone 各占 1 列
- **空 Zone 不渲染卡片**——当某个 Zone 获得第一件装备时，卡片以动画出现（渐进式解锁）
...
- **触控规范**：badge 可见区域至少 20×20vp，加 padding 达到 48×48vp 触控热区，使用 `hitTestBehavior(HitTestMode.Block)` 防止穿透到装备行的 tap 勾选
...
**核心规则**：
- 无装备时不显示任何 Zone 卡片、不显示进度条
- 居中引导按钮（非 FAB）：视觉焦点集中，用户不会困惑
- 用户添加第一件装备后，切换为正常布局（Zone 卡片 + FAB + 进度条）
...
- 当某个 Zone 获得第一件装备时，卡片以 staggered 动画出现（scale + fadeIn）
...
|  |  [search] 搜索装备...                     ||
|  +------------------------------------------+|
|  全部  穿着·上身  饮食  电力  ...            |  <- CategoryTagGroup 品类筛选
|                                              |
...
tap GearPickerSheet 底部「+ 临时添加」→ 弹出 mini dialog（覆盖在 Sheet 之上）：

```
+----------------------------------------------+
|  临时添加装备                                 |
|                                              |
|  名称  [_______________]                     |  <- 必填
|                                              |
|  归属区域                                    |
|  [头部] [上身] [下身] [脚部]                 |  <- Zone chips（必选，默认杂项）
|  [背负] [睡眠] [杂项*]                       |
|                                              |
|  重量（可选） [_____] g                      |
|                                              |
...
- tap「添加」→ 创建 ChecklistItem（`fromGearId` 为空，`group` = 选中 Zone）→ dialog 关闭 → 回到 GearPickerSheet
...
|                                              |
|    +--------------------+                    |
|    |  去装备库           |                    |  <- tap -> 关闭 Sheet -> 切换到装备库 Tab
|    +--------------------+                    |
|                                              |
|    或 临时添加一件装备                        |  <- 链接，tap -> 弹出临时添加 dialog
+----------------------------------------------+
```
...
| 添加/删除装备 | ✅ 是 |
| 勾选/取消勾选 | ✅ 是 |
| 修改层级 badge | ✅ 是（即使改回原值也算，简化实现） |
| 移动装备到其他 Zone | ✅ 是 |
| 仅切换视图未做操作 | ❌ 否 |
| 打开 GearPickerSheet 又关闭未选 | ❌ 否 |
| 打开 long-press 菜单未执行操作 | ❌ 否 |
...
│   └── TempGearMiniSheet.ets      # 临时添加装备 mini sheet
└── services/
...
- 有装备时：进度条 + Zone 卡片网格（仅展示有装备的 Zone）+ FAB
...
- long-press → 展开操作菜单
- 层级 badge 可见区域 ≥ 20×20vp，padding 扩展到 44×44vp 触控区（`hitTestBehavior(HitTestMode.Block)` 防穿透）
...
- 布局：搜索栏 → 品类 tab → **已选汇总栏** → 装备列表 → 临时添加入口
- 已选汇总栏：`已选 N 件 ▸ 上身 2 · 杂项 1`（实时更新，显示 Zone 分布）
- 每行装备右侧：目标 Zone chip（使用 ZONE_*_COLOR 对应颜色）
- 选中反馈：Zone chip 变绿 + ✓ + scale bounce
- **连续多选**：选中后 Sheet 不关闭
- 关闭 Sheet 后：新添加的装备在配装视图中以 staggered 入场动画一起出现
- **装备库为空态**：中央 icon + 文案 + 双入口（「去装备库」+「临时添加一件」）

#### TempGearMiniSheet（临时添加）
- 从 GearPickerSheet 内部呼出（覆盖在其上）
- 三个字段：名称（必填）+ 归属区域（Zone chips，默认 Misc）+ 重量（可选）
- 确认后创建 ChecklistItem（fromGearId 为空，group = 选中 Zone）
- 关闭后回到 GearPickerSheet
...
| Sheet 呼出 | 通过 SheetOverlay 路由（新增 `SHEET_GEAR_PICKER` / `SHEET_TEMP_GEAR` 常量） |
...
- FAB：右下角悬浮，主题绿 `PRIMARY_COLOR` 圆形，56×56vp，仅配装视图可见
...
Badge 仅在 UpperBody / LowerBody 卡片内显示。触控区域：可见区域 ≥ 20×20vp + padding 扩展到 44×44vp。
...
**空 Zone 不渲染卡片**（见 §3.3 渐进式策略），因此不存在单个 Zone 的空态显示。
...
| 场景 | 动画类型 | 参数 |
|------|----------|------|
| Zone 卡片首次出现 | staggered scale(0.9→1) + fadeIn | 间隔 40ms，Spring(0.35, 0.8) |
| 装备项入场（Sheet 关闭后） | staggered translateY(12→0) + fadeIn | 间隔 30ms，Spring(0.3, 0.75) |
| 勾选切换 | ✓ icon 瞬间出现 + bgColor 快速过渡 | 50ms EaseOut → Spring(0.25, 0.7) 收尾 |
...
| GearPickerSheet 内选中 | Zone chip 色变 + ✓ + scale bounce | Spring(0.25, 0.7) |
...
| 首次引导 → 有装备状态 | 引导区 fadeOut + 首个 Zone 卡片 fadeIn | Spring(0.35, 0.8) |
...
GearPickerSheet 底部「+ 临时添加」→ 弹出 TempGearMiniSheet：
- 名称（必填 TextInput）
- 归属区域（Zone chips，默认 Misc）
- 重量（可选 TextInput，单位 g）
- 确认 → 创建 ChecklistItem（fromGearId 空，group = 选中 Zone）→ mini sheet 关闭 → 回到 GearPickerSheet
...
中央 icon + 文案「装备库还是空的」+ 双入口：
- 「去装备库」按钮 → 关闭 Sheet → 切换到装备库 Tab
- 「临时添加一件装备」链接 → 呼出 TempGearMiniSheet

### 8.6 自定义品类的装备如何落槽？

用户通过 CategoryTagGroup 创建的自定义品类（如「滑雪」「攀岩」），不在 CATEGORY_SLOT_MAP 中。`slotHintForCategory()` 返回 fallback：`{ zone: Misc, layer: Accessory }`。用户可通过 long-press 移动到合适的 Zone。
...
3. `TripDetailPage` 内部通过 @State 控制显示 `LoadoutView` 或 `ChecklistDetail`
4. `.geometryTransition('trip-' + id)` 绑在这个新 NavDestination 上
5. `.onBackPressed()` 在新 NavDestination 上处理条件保存 + pop
6. 原 `'ChecklistDetail'` 分支保留作为 fallback（feature flag 切换），稳定后移除
...
- [x] LoadoutView 主视图（2 列 Zone 卡片网格，渐进式出现）
- [x] LoadoutZoneCard + LoadoutGearItem 组件
- [x] Misc 卡片内部按品类分子区
- [x] GearPickerSheet（选装 + 目标 Zone 标签 + 已选汇总栏 + 连续多选）
- [x] TempGearMiniSheet（临时添加完整流程）
- [x] tap 勾选（即时反馈）+ long-press 菜单（移动/详情/移除）
- [x] 进度指示（已装包 N/M + 100% 庆祝）
- [x] 自动落槽（CATEGORY_SLOT_MAP）
- [x] FAB 添加按钮（仅配装视图）
- [x] Zone 颜色系统 + icon 标识
- [x] 层级 badge（仅衣物 Zone）+ tap 切换层级 + 44×44vp 触控区
- [x] 空态设计（全屏引导 → 渐进式 Zone 出现 / Sheet 空态双入口）
...
Phase 1: 基础层
  ...
  └── LoadoutGearItem.ets（最小渲染单元，含层级 badge + 勾选即时反馈）

Phase 2: 容器组件
  ├── LoadoutZoneCard.ets（依赖 LoadoutGearItem，含 Misc 子分区逻辑）
  └── LoadoutProgressBar.ets

Phase 3: Sheet
  ├── GearPickerSheet.ets（依赖 CategoryTagGroup + 已选汇总栏 + Zone chip）
  └── TempGearMiniSheet.ets（临时添加 mini sheet）

Phase 4: 主页面集成
  ├── LoadoutView.ets（配装视图，组装 ZoneCard 网格 + FAB + 空态引导）
  ├── TripDetailPage.ets（双视图容器 + 共享信息区 + SegmentButton）
  ├── Index.ets NavDestinationMap 新增 'TripDetailPage' 分支
  └── HomePage 行程 tap 改为 pushPathByName('TripDetailPage')

Phase 5: 转场 & 动效打磨
  ├── geometryTransition 迁移到新 NavDestination
  ├── Zone 卡片入场动画
  ├── 勾选即时反馈
  ├── FAB 显示/隐藏动画
  ├── 视图切换交叉淡入淡出
...
6. **动效到位**：所有交互均有 Spring 弹性响应，无硬切；勾选即时可感知
7. **空态优雅**：新行程全屏引导 → 添加首件装备后 Zone 卡片渐进出现
8. **性能达标**：50+ 装备项的行程渲染无掉帧（保持 60fps）
9. **核查复盘可达**：两个视图均可通过 `···` 菜单进入 ReviewPage
10. **临时添加跑通**：GearPickerSheet → 临时添加 → Zone 卡片出现临时装备
11. **自动保存正确**：有变更退出 → 数据持久化；无变更退出 → 不触发 I/O
12. **体验验证**：配装过程有「像玩游戏」的感觉，不是在「填表格」
...
| 自动落槽准确率不高 | 用户困惑 | Sheet 内 Zone chip 预览 + long-press 移动 |
...
|------|------|------|
| v1 | 2026-06-10 | 初稿 |
| v2 | 2026-06-10 | 吸收第一轮审查：P0×3 + P1×3 + P2×5 |
| v3 | 2026-06-10 | 吸收第二轮审查：P0×1 + P1×5 + P2×7，全流程走查补全 |
...
9. **[P2] Badge 触控区域**：明确 44×44vp 最小触控区 + hitTestBehavior 防穿透。
