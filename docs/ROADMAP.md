...
> **来源**：产品愿景中的行程规划功能落地，2026-06-24 完成（10 Phase + 1 轮全面审查优化）。
...
- [x] **ItineraryService CRUD 纯函数**：`addDay`/`removeDay`/`patchDay`/`addSegment`/`removeSegment`/`patchSegment` + `DayPatch`/`SegmentPatch` 类型定义
...
GearPage 当前 ~2780 行，38 个 @State，10 个 @Prop/@StorageProp，承担 8 个独立子系统。需要将自包含的子系统提取为独立组件，降低耦合度和单个文件复杂度。
