...
> **来源**：`docs/v2-foundation/plans/2026-06-22-checklist-visual-polish-benchmark.md`（审查通过，待排期）
...
- [ ] **Phase B — 容器化**：满格子白卡加 `ZONE_*_STROKE` 极淡描边 + 阴影升级 + 标题行浅染 `ZONE_*_TINT` 背景；空格子压缩淡化 + 提示文案；列高对齐
- [ ] **Phase C — 拖动物理感深化**：源位虚线空槽 + 目标格高亮微调
- [ ] **Phase D — 展开态+转场重构**：抽共享 Zone 外壳 → 展开态改卡片浮层（不铺满全屏）→ 背景联动压暗/缩放 → 真机录屏对比便单
- [ ] **Phase E — 顶部信息区 + 进度条 + 勾选态**：元信息行图标点睛 + 进度条轻量升级 + 勾选完成态强化

### 参考文档
- `docs/v2-foundation/plans/2026-06-22-checklist-visual-polish-benchmark.md`
...
| v0.7.1 | 🔵 短期 | UI 质感提升：容器化 + 转场重构 + 拖动深化 |
