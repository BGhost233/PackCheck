...
> 最后更新：2026-06-24 | 当前版本：v0.7.8-itinerary（已完成）
> 2026-06-24 行程编辑模块完整落地：Tabs 滑动切换 + ItineraryService CRUD + DayCard 手风琴 + SegmentFormSheet/DayFormSheet 表单 + 交互动效全面审查优化。
...
- 原设计 spec 文件（unified-checklist-view-design / product-vision / service-archive-restructure）已在 v0.6.1 文档重组时归档删除，核心内容整合在本路线图及 DEVELOPMENT_STANDARDS 中
...
### 参考文档
- `docs/PERF_OPTIMIZATION_PLAN.md`（完整方案 + 预估收益，8 步全部实施完毕，2026-06-23 二次审计确认）

---

## 🟢 已完成（v0.7.7-cleanup）—— 2026-06-23 全面代码审查 & 清理

> **来源**：全项目代码逐行审查（55 .ets 文件，~64,000 行），结合鸿蒙文档/避坑清单/开发规范。
> **原则**：最小改动、向后兼容、每步构建验证 + commit。

### P0 安全修复

- [x] **setTimeout 泄漏修复**：7 文件（TripCeremonyCard 12+ timer / ReviewPage 3 / FocusedZoneView 2 / GearDetailSheet / ZoneGridCell / LoadoutProgressBar / GearPage 4）补 `aboutToDisappear` + `isAlive` + `timerIds[]` 体系
- [x] **PRESS_SCALE_BOUNCE 中性值保护**：TripCeremonyCard aboutToDisappear 防御性重置 4 弹性 scale → 1.0
- [x] **`'其他'` → `CATEGORY_FALLBACK`**：ChecklistService + LoadoutService 硬编码替换为常量引用

### P1 缺陷修复

- [x] **Record<string,number> → 安全类型**：UnifiedChecklistView 2 处 onAreaChange 消除不安全转换
- [x] **GearPage 死代码清理**：5 死方法 + 2 未用 import 删除
- [x] **ColorUtils hex → token**：`#2D7D46`→`PRIMARY_COLOR`，heroGradientStart 引用 `PRIMARY_TINT_COLOR`/`LIGHT_PRIMARY_COLOR`/`AMBER_TINT`
- [x] **硬编码色值 → token**：GearItemContextMenu `#F2FFFFFF`→`GLASS_BG_LIGHT`，ReviewPage 尺寸→模块常量

### P2 性能 & 质量

- [x] **AnimationUtils 死代码**：280→78 行（16→3 导出，88% 切除）
- [x] **HapticUtils 死代码**：76→52 行（删除 3 未消费导出）
- [x] **services 死代码三文件**：ChecklistService 601→441 行 + GearService 383→301 行 + LoadoutService 414→205 行（共 27 死函数/453 行删除）
- [x] **constants 死 token**：AnimationTokens 9 + Colors 2 + DesignTokens 4 删除
- [x] **PERF_OPTIMIZATION_PLAN 落实验证**：全部 8 项确认已实施，文档标注完成

### 文档同步

- [x] **避坑清单数字**：CLAUDE.md 46→48（与 memory/MEMORY.md 对齐）
- [x] **README 架构图**：Index 行号/service 描述/AnimationUtils/HapticUtils/docs 目录全部同步代码状态
- [x] **ROADMAP 更新**：日期→2026-06-23，GearPage 行号修正 2780/38@State

### 构建 & 代码量

- **构建**：全程 BUILD SUCCESSFUL
- **代码量**：21 files，+290/-1076（净删 786 行）
- **提交数**：19 commits

---

## 🟢 已完成（v0.7.8）—— 行程编辑模块（Tab 滑动 + CRUD + 动效）

> **来源**：产品愿景中的行程规划功能落地，2026-06-24 完成（10 Phase + 1 轮全面审查优化 + 8 个增量审计修复 commit + 1 轮全量代码审查清理）。

### 背景

行程详情页原为单一装备核查清单视图。v0.7.8 新增「行程」Tab，支持按天/路段录入行程规划，与「装备」Tab 平行滑动切换，数据持久化驱动装备→行程关联闭环。

### 任务清单

- [x] **Tabs 替换 Stack+if**：使用原生 Tabs 组件实现左右滑动切换，`onGestureSwipe` 跟手插值 Tab 标题颜色渐变 + `SPRING_TAB(0.32, 0.82)` 加速切换
- [x] **ItineraryService CRUD 纯函数**：`addDay`/`insertDay`/`removeDay`/`updateDay`/`addSegment`/`removeSegment`/`updateSegment`/`getDaySummary`/`getTransportIcon`/`createEmptySegment` + `DayPatch`/`SegmentPatch` 类型定义
- [x] **ItineraryView 列表容器**：ForEach 按天渲染 DayCard，手风琴展开同时只展开一天
- [x] **DayCard 日卡片**：手风琴折叠展开（asymmetric transition）+ 段行（城市→城市 + 交通图标 + 时间摘要）+ cachedSummary @Watch 缓存
- [x] **SegmentFormSheet / DayFormSheet**：表单面板组件，通过 SheetOverlay 路由
- [x] **可编辑模式**：编辑回调 + 添加路段节点 + 长按菜单
- [x] **CRUD 数据贯通**：Index → TripDetailPage → ItineraryView → DayCard 回调链完整对接 + ItineraryService 持久化
- [x] **全面审查优化**（commit `a705cf0`）：onGestureSwipe progress 映射修正、Tab 标题 hex 通道插值、longPressTriggered 防双击、head collapse animateTo 包裹、段行 pressed 高亮 + stateStyles、展开 asymmetric transition、Tab 按压反馈 scale
- [x] **行程详情页审计修复**（8 个增量 commit）：DayItinerary from/to 字段解耦、表单 Sheet 正式接入、renderNonce 透传修复 UI 不刷新、ForEach key 稳定化、空态增强、聚焦态边框、入场动效、Tab 双侧下压
- [x] **全量代码审查清理**（commit `bded20e`）：28 文件净删 786 行（死代码/无用 import/浮动 Promise/hardcode 色值/timer 泄漏/DesignTokens 清理）

### 关键技术决策

- Tab 标题颜色：从阈值硬切改为 RGB hex 通道插值（`#1A1A1A` ↔ `#999999`），跟手连续
- onGestureSwipe currentOffset 语义：负值 = 内容左移 = 朝 index+1；progress = `tabIndex - offset / tabsWidth`
- DaySummary 缓存：`@Prop @Watch('onDayChange') day` 触发 `cachedSummary` 重算，build 中 O(1) 读取
- 长按/点击冲突：复用避坑 #45 的 `longPressTriggered` flag 模式
...
GearPage 当前 ~2810 行，40 个 @State，10 个 @Prop/@StorageProp，承担 8 个独立子系统。需要将自包含的子系统提取为独立组件，降低耦合度和单个文件复杂度。
...
| v0.7.7-cleanup | ✅ 已完成 | 全面代码审查 & 清理（死代码/timer泄漏/硬编码token化，净删 786 行） |
| v0.7.8 | ✅ 已完成 | 行程编辑模块（Tabs 滑动 + ItineraryService CRUD + DayCard 手风琴 + 表单 + 动效审查优化） |
...
| ArkUI 避坑 | 50 条实战踩坑记录（动效/手势/布局问题先查） | `memory/MEMORY.md` |
