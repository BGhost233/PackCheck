...
### 23. Navigation Stack 模式下覆盖层必须在 Navigation 外层
- Navigation 的 NavDestination 渲染时会覆盖 Navigation 内部的 Stack 子组件（包括 Sheet、弹窗、仪式卡片等覆盖层）
- 表现：覆盖层代码触发但用户看不到——因为它被 NavDestination 全屏遮挡在下层
- 正确做法：用外层 Stack 包裹 Navigation，覆盖层放在 Navigation **同级之后**（zIndex 更高），这样它们渲染在 NavDestination 之上
- 结构模式：`Stack { Navigation {...} SheetOverlay() TripCeremonyCard() }` — 不是 `Navigation { Stack { ... SheetOverlay() } }`
- 附带：外层 Stack 需要 `.expandSafeArea()` 继承沉浸式布局

### 24. ForEach key 必须包含所有影响渲染的变化维度
- ForEach 的 `keyGenerator` 不仅用于 diff 列表项增删，更决定了**已有项是否触发重渲染**
- 如果 key 只含 `item.id`，即使 item 内部属性变化（如 items.length / checked 状态），ArkUI 会跳过该项的 @Builder 重建
- 正确做法：key 中拼入会影响 UI 的关键属性，如 `item.id + '_' + item.items.length + '_' + nonce`
- 典型场景：父组件 `@State` 更新 → `@Prop` 深拷贝传给子组件 → 子组件 ForEach key 不变 → UI 不刷新
- nonce 模式：父组件维护 `checklistRenderNonce` 计数器，数据变更时递增，子组件将 nonce 拼入 key 强制刷新

### 25. 搜索态必须绕过折叠/隐藏状态
- 列表有分组折叠功能时，搜索匹配到的内容如果在折叠分组中，用户看不到结果
- 正确做法：状态查询方法（如 `isGearGroupCollapsed()`）在搜索关键词非空时直接返回 `false`
- 规则：**搜索态 = 全部展开**，不依赖 UI 层做额外展开逻辑，从数据源头绕过
- 推广：任何「过滤/搜索」场景都要检查是否有「折叠/隐藏」状态与之冲突

### 26. PanGesture distance 阈值防止吃掉 onClick
- `PanGesture({ distance: 0 })` 会让手指任何微小移动（1px）都触发 Pan 手势，导致同一组件的 `.onClick()` 永远不触发
- 典型表现：多选模式下点击行应该 toggle 选中，但 Pan(0) 先于 Click 消费了事件
- 正确做法：`PanGesture({ distance: 5 })`，5vp 的容差让轻点正常走 onClick，只有真拖动才激活 Pan
- 规则：当 PanGesture 和 onClick 在同一组件上共存时，distance **至少 5**

### 27. if/else 条件渲染的展开/折叠动画
- ArkUI 的 `if (condition) { ... }` 默认硬切（瞬间出现/消失），没有过渡动画
- 要实现展开/折叠的平滑过渡必须**两步配合**：
  1. 用 `animateTo(spring)` 包裹控制 condition 的 state 赋值
  2. 被 if/else 控制的组件加 `.transition(TransitionEffect.OPACITY.combine(TransitionEffect.translate({ y: -6 })))`
- 缺一不可：只有 animateTo 没有 .transition → 无效；只有 .transition 没有 animateTo → 也无效
- 适用场景：分组内容折叠展开、搜索框展开/收起、模式切换时的组件进退场