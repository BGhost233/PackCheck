...
**⚠️ 每次新会话开始，第一件事必须用 Read 工具完整阅读 `docs/vision/2026-06-04-product-vision-and-restructure.md`（当前为 v2 版本）全文。** 这是 PackCheck 从「装备清单 App」向「装备的服役档案」转型的纲领性文档，定义了：
...
- 配装系统的核心交互范式（塔科夫式槽位、横轴身体部位 × 纵轴分层、容器即实例、两阶段流水线）
- 四 Tab 重构蓝图 + 三步走落地排序 + 被推翻/被否决方案清单（含 v1→v2 范式转移记录）
