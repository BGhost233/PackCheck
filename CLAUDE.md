...
2. **`memory/MEMORY.md` 的「ArkUI 避坑清单」全部 48 条**（不是遇到问题再查，是开场就读完）
...
存于 `memory/MEMORY.md`「ArkUI 避坑清单」section（共 48 条）。**已在会话启动第一动作中要求完整读取全部 48 条**——这些坑都是踩过的血泪，开场读完才能在写代码时主动规避，而不是踩了再回查。