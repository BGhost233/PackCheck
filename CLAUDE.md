...
**⚠️ 会话启动第一动作（不可绕过）：每次新会话开始、回复任何内容或写任何代码之前，必须先用 Read 工具完整阅读以下两份文件，缺一不可——**

1. **`docs/DEVELOPMENT_STANDARDS.md` 全文**
2. **`memory/MEMORY.md` 的「ArkUI 避坑清单」全部 50 条**（不是遇到问题再查，是开场就读完）

DEVELOPMENT_STANDARDS 定义了：
...
- **结构防腐铁律（第八章）**——防上帝组件、防跨文件重复、防封装脱节、防死代码、防文件位置混乱，含拆分阈值表、「何时不该拆」清单、grep-before-add 铁律、提交前自查清单
...
存于 `memory/MEMORY.md`「ArkUI 避坑清单」section（共 50 条）。**已在会话启动第一动作中要求完整读取全部 50 条**——这些坑都是踩过的血泪，开场读完才能在写代码时主动规避，而不是踩了再回查。